#!/usr/bin/env python3
"""把《加快建设世界一流零碳总部_南网总部基地方向稿_2026-09-03.md》转成公文体例的 Word 文稿。

用法：
    python3 build_docx.py            # 在本目录生成同名 .docx

只依赖 python-docx（pip install python-docx）。Markdown 只使用有限子集：
# / ## / ### / #### 标题、段落、- 无序列表、1. 有序列表、| 表格 |、--- 分隔线、**加粗**。

版式：A4；正文仿宋（仿宋_GB2312）三号；篇级标题黑体小二居中另起页、
一级标题黑体三号、二级标题仿宋三号加粗；封面标题方正小标宋；表格宋体小五；
页脚页码；封面后附目录域（Word 中右键“更新域”生成）。
"""
...
STEM = "加快建设世界一流零碳总部_南网总部基地方向稿_2026-09-03"
SOURCE = HERE / f"{STEM}.md"
OUTPUT = HERE / f"{STEM}.docx"
...
def build(source: Path = SOURCE, output: Path = OUTPUT) -> Path:
    ...
    while i < len(lines):
        ...
        text = "".join(buf)
        p = doc.add_paragraph()
        is_signature = text.startswith("**起草单位**") or text.startswith("**稿件日期**")
        para_format(
            p,
            first_line_indent=not is_signature,
            align=WD_ALIGN_PARAGRAPH.RIGHT if is_signature else None,
            space_after=6,
        )
